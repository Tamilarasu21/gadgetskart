...
<body bgcolor="#f9f9f9">
    <div class="main">
        <?php
            ...
            elseif(strpos($Url,"add=empty")==true)
            {
                echo '<div class="alert alert-danger alert-dismissible text-center">
                        <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
        ...
        ?>
        <div class="container">
            <form action="" method="post" enctype="multipart/form-data" class="well form-horizontal">
                <legend><h1 class="text-success text-center">Add Your Product</h1></legend>
                <!-- product name -->
                    ...
                    <div class="col-md-4 inputGroupContainer">
                        <div class="input-group">
                            <?php
                                ...
                                {
                                    $path=$_GET['path'];
                                    echo '<input type="file" name="image" id="image" class="form-control-file" accept=".png" value="'.$path.'">';
                                    echo '<img src="'.$path.'">';
                                }
                                else
                                {
                                    echo '<input type="file" name="image" id="image" class="form-control-file" accept=".png">';
                                }
                            ?>
                        </div>
                    </div>
                ...
                <div class="form-group">
                    ...
                    <div class="col-md-2 inputGroupContainer">
                        <div class="input-group">
                            <button name="add" class="btn btn-success btn-block"><i class="fa fa-plus"></i>&emsp;Add</button>
                        </div>
                    </div>
                </div>
                </td></tr>
...
<?php
    ...
    if(isset($_POST['add']))
    {
        ...
        $aprice=stripslashes($_POST['aprice']);
        $oprice=stripslashes($_POST['oprice']);

        $image=$_FILES['image'];
        $imagePath=$_FILES['image']['name'];
        $imageTmp=$_FILES['image']['tmp_name'];
        $imageNameNew=uniqid('',true).".".$imagePath;
        $imageDest="images/".$imageNameNew;
        move_uploaded_file($imageTmp,$imageDest);

        $description=stripslashes($_POST['description']);
        $quantity=stripslashes($_POST['quantity']);
        $user=stripslashes($_SESSION['seller']);
        if(empty($product_name) || empty($category) || empty($brand) || empty($color) || empty($aprice) || empty($oprice) || empty($imageDest) || empty($description) || empty($quantity) || empty($user))
        {
            header("Location:addProduct.php?add=empty&product_name=$product_name&category=$category&brand=$brand&color=$color&aprice=$aprice&oprice=$oprice&path=$imageDest&description=$description&quantity=$quantity&user=$user");
        }
        else
        {
            $sql="insert into products (product_name,category,brand,color,aprice,oprice,path,description,quantity,user) values ('$product_name','$category','$brand','$color','$aprice','$oprice','$imageDest','$description','$quantity','$user')";
            ...
            {
                header("Location:addProduct.php?add=failed&product_name=$product_name&category=$category&brand=$brand&color=$color&aprice=$aprice&oprice=$oprice&path=$imageDest&description=$description&quantity=$quantity&user=$user");
            }
        }
    }
?>